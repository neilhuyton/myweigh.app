...
test.describe("Weight Goal Functionality", () => {
  test("should set and display weight goal when logged in", async ({
    ...
    // Verify initial goal
    await expect(page.getByText("Current Goal: 65 kg")).toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(
      page.getByPlaceholder("Enter your goal weight (kg)")
    ).toHaveValue("60");
    ...
    await expect(page.getByText("Current Goal: 60 kg")).toBeVisible({
      timeout: 20000,
    });
  });
  ...
  test("should display error message for invalid goal weight", async ({
    ...
    await expect(page.getByText("Goal set successfully!")).not.toBeVisible({
      timeout: 5000,
    });
  });
});
