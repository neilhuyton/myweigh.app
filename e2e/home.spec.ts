...
test.describe("Home Page", () => {
  ...
  test("should navigate to correct routes when dashboard card buttons are clicked", async ({
    page,
  }) => {
    ...
    await page.route(
      "**/trpc/weight.getWeights,weight.getGoal**",
      async (route) => {
    ...
    await expect(page.getByText("Weight Tracker Dashboard")).toBeVisible({
      timeout: 10000,
    });
    await expect(page.getByTestId("current-weight-card")).toBeVisible({
      timeout: 10000,
    });
    await expect(page.getByTestId("goal-weight-card")).toBeVisible({
      timeout: 10000,
    });
    await expect(page.getByTestId("weight-change-card")).toBeVisible({
      timeout: 10000,
    });
    await expect(page.getByTestId("recent-measurement-card")).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(page.getByTestId("goal-weight-card-button")).toBeVisible({
      timeout: 10000,
    });
    ...
    await page.goto("/"); // Navigate back to Home
    await expect(page.getByTestId("weight-change-card-button")).toBeVisible({
      timeout: 10000,
    });
    ...
    await page.goto("/"); // Navigate back to Home
    await expect(
      page.getByTestId("recent-measurement-card-button")
  ...
  test("should display profile icon when logged in and navigate to profile", async ({
    page,
  }) => {
    // Mock the tRPC login request
    await page.route("**/trpc/login**", async (route) => {
  ...
  test("should not display profile icon when not logged in", async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId("profile-icon")).not.toBeVisible({
      timeout: 5000,
    });
  });

  // e2e/home.spec.ts
  test("should handle email update form on profile page", async ({ page }) => {
    // Mock the tRPC login request
    await page.route("**/trpc/login**", async (route) => {
      if (route.request().method() === "POST") {
        await route.fulfill({
    ...
    // Mock the tRPC updateEmail request
    let oldEmailNotified: string | null = null;
    await page.route("**/trpc/user.updateEmail**", async (route) => {
      if (route.request().method() === "POST") {
        const headers = await route.request().allHeaders();
        if (headers["authorization"] !== "Bearer test-user-id") {
          await route.fulfill({
          ...
          const { email } = JSON.parse(postData || "{}");
          oldEmailNotified = "testuser@example.com"; // Simulate notification to old email
          await route.fulfill({
            status: 200,
            contentType: "application/json",
            headers: {
    ...
    await expect(page.getByTestId("email-success")).not.toBeVisible({
      timeout: 5000,
    });
    ...
    await Promise.all([
      page.waitForResponse(
        (resp) =>
          resp.url().includes("trpc/user.updateEmail") && resp.status() === 200,
        { timeout: 20000 }
      ),
      page
        .getByTestId("email-form")
    ...
    await page
      .getByTestId("email-success")
      .waitFor({ state: "visible", timeout: 10000 });
    await expect(page.getByTestId("email-success")).toHaveText(
    ...
    // Verify that the old email was "notified"
    expect(oldEmailNotified).toBe("testuser@example.com");
  });

  test("should handle password reset form on profile page", async ({
    page,
  }) => {
    // Mock the tRPC login request
    await page.route("**/trpc/login**", async (route) => {
    ...
    // Mock the tRPC resetPassword.request
    await page.route("**/trpc/resetPassword.request**", async (route) => {
      if (route.request().method() === "POST") {
        const headers = await route.request().allHeaders();
        if (headers["authorization"] !== "Bearer test-user-id") {
          await route.fulfill({
    ...
    // Test invalid email
    await page.getByTestId("password-input").fill("invalid-email");
    await page
      .getByTestId("password-form")
      .evaluate((form) => (form as HTMLFormElement).requestSubmit());
    await page
    ...
    await expect(page.getByTestId("password-success")).not.toBeVisible({
      timeout: 5000,
    });

    // Test valid email
    await page.getByTestId("password-input").fill("testuser@example.com");
    await Promise.all([
      page.waitForResponse(
        (resp) =>
          resp.url().includes("trpc/resetPassword.request") &&
          resp.status() === 200,
        { timeout: 20000 }
      ),
      page
    ...
    await page
      .getByTestId("password-success")
      .waitFor({ state: "visible", timeout: 10000 });
    await expect(page.getByTestId("password-success")).toHaveText(
    ...
    await expect(page).toHaveURL(/.*\/profile/, { timeout: 10000 }); // Verify we stay on /profile
  });
});
