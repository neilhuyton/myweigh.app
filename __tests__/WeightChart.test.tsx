...
describe("WeightChart Component", () => {
  ...
  const setup = async (
    initialPath: string = "/weight-chart",
  ...
  it.each([
    ...
    "displays weight chart with correct theme color ($theme)",
    async ({ theme, expectedColor }) => {
      ...
      await waitFor(() => {
        ...
        const card = screen
          .getByTestId("chart-container-mock")
          .closest(".bg-card");
        expect(card).toHaveClass("overflow-hidden");
        expect(screen.getByTestId("chart-container-mock")).toHaveClass(
          "h-full w-full"
        );
        expect(screen.getByTestId("chart-container-mock")).toHaveAttribute(
          "data-chart",
          "weight-chart"
        );
        expect(screen.getByTestId("chart-mock")).toBeInTheDocument();
        const chartData = JSON.parse(
          screen.getByTestId("chart-mock").getAttribute("data-chart-data") ||
            "[]"
        );
        ...
        const config = JSON.parse(
          screen
            .getByTestId("chart-container-mock")
            .getAttribute("data-chart-config") || "{}"
        );
